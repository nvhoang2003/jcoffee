...
//            statement.setInt(3, orders.getAmount());
    ...
    public static List<Bill> ListPendingOrder() {
        ...
        String sql = "SELECT o.order_id as order_id, c.customer_name as customer_name, t.name as table_name, SUM(od.price) as amount "
                + "FROM orders as o "
                + "JOIN customers as c ON o.customer_id = c.customer_id "
                + "JOIN tables as t ON o.table_id = t.table_id "
                + "JOIN order_drinks as od ON o.order_id = od.order_id "
...
//            statement.setInt(1,OrderState.getKeyByValue("Pending"));
            ResultSet rs = statement.executeQuery();
            while (rs.next()) {
                Bill bill = new Bill(rs.getInt("order_id"), rs.getInt("amount"), rs.getString("customer_name"), rs.getString("table_name"));
                list.add(bill);
            }
        } catch (SQLException ex) {
            Logger.getLogger(OrdersDAO.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static void setComplete(int id) {
        ...
        try {
            statement = conn.prepareStatement(sql);
            statement.setInt(1, OrderState.getKeyByValue("Completed"));
